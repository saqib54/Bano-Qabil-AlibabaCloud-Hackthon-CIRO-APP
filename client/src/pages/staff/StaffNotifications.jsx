            ...
            {['ALL', 'UNREAD'].map((f) => (
              <button
                ...
                className={`px-3 py-1.5 text-xs font-medium transition ${
                  filter === f ? 'bg-brand text-white' : 'bg-white text-ink-soft hover:bg-surface'
                }`}
              >
                {f === 'ALL' ? 'All' : 'Unread'}
              </button>
      ...
      {loading ? (
        ...
        <div className="card p-12 text-center">
          ...
          <p className="mt-1 text-xs text-ink-soft">
            You'll receive notifications for new assignments, status changes, and emergency broadcasts.
          </p>
        </div>
      ) : (
              ...
              <div className="min-w-0 flex-1">
                <div className="flex items-start justify-between gap-2">
                  <div>
                    ...
                    {n.type && (
                      <span className="mt-0.5 inline-block rounded bg-surface px-1.5 py-0.5 text-[9px] font-medium uppercase tracking-wide text-ink-soft">
                        {n.type}
                      </span>
                    )}
                  </div>
