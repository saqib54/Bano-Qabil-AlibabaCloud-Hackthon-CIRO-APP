...
import { CloudSun, Search, Loader2, Gauge, Sunrise, Lightbulb, Sparkles, MapPin } from 'lucide-react';
...
/**
 * Citizen Weather Intelligence — AI meteorologist briefing, 3D hero card,
 * severe alerts, 15-day outlook and live radar layers.
 */
      ...
      <div className="mx-auto max-w-4xl px-4 pt-8">
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
          <div>
            ...
            <p className="mt-1 text-sm text-white/50">AI meteorologist briefing for your city — alerts, forecast & live radar.</p>
          </div>
        ...
        {!loading && data && (
          <>
            ...
            {/* Conditions strip: AQI, astronomy, key metrics */}
            ...
            {/* AQI health recommendation */}
            {data.aqi?.recommendation && (
              <div className="mx-auto mt-4 max-w-4xl px-4">
                <div className="flex items-start gap-3 rounded-2xl border border-orange-400/20 bg-orange-500/10 p-4 backdrop-blur-xl">
                  <Gauge className="mt-0.5 h-5 w-5 shrink-0 text-orange-300" />
                  <div>
                    <p className="text-[10px] font-black uppercase tracking-widest text-orange-300">Air quality guidance</p>
                    <p className="mt-1 text-xs leading-relaxed text-white/80">{data.aqi.recommendation}</p>
                  </div>
                </div>
              </div>
            )}
            ...
            {data.insights?.length > 0 && (
              <div className="mx-auto mt-8 max-w-4xl px-4">
                <h3 className="mb-4 flex items-center gap-3 pl-2 text-lg font-black uppercase tracking-widest text-white/80">
                  <div className="p-2 rounded-lg bg-aqua/20"><Lightbulb size={18} className="text-aqua" /></div> Lifestyle Insights
                </h3>
                <div className="grid grid-cols-1 gap-3 sm:grid-cols-2 lg:grid-cols-4">
                  {data.insights.map((ins, i) => (
                    <div key={i} className="rounded-2xl border border-white/10 bg-white/5 p-4 backdrop-blur-xl transition hover:bg-white/10">
                      <p className="text-sm font-black text-white">{INSIGHT_ICON[ins.category] || <Sparkles className="inline h-4 w-4 text-aqua" />} {ins.category}</p>
            ...
            <p className="mx-auto mt-2 max-w-4xl px-4 text-center text-[10px] font-bold uppercase tracking-widest text-white/30">
              {data.source === 'ai' ? 'Generated by CIRO AI meteorologist (Qwen)' : 'Offline model — AI feed unavailable, showing seasonal estimate'} · For emergency weather events, report via CIRO
            </p>
          </>
        )}
      </div>
